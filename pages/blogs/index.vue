...
const { data } = await useAsyncData("blogs", () =>
  queryContent("/blogs").where({ published: true }).sort({ $numeric: true, date: -1 }).find(),
)
...
const totalPages = computed(() => Math.ceil(searchResults.value.length / elementPerPage.value))
  ...
  <main class="container max-w-5xl mx-auto text-zinc-600">
    ...
    <div class="px-6">
      <input
        ...
        class="block w-full bg-[#F1F2F4] dark:bg-slate-900 dark:placeholder-zinc-500 text-zinc-600 dark:text-zinc-300 rounded-md border-gray-300 dark:border-gray-800 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 px-4 py-2"
      />
    </div>
    ...
    <div v-auto-animate class="space-y-5 my-5 px-4">
      <template v-if="paginatedData.length">
        <ArchiveCard v-for="post in paginatedData" :key="post.path" v-bind="post" />
      </template>
      <ArchiveCard v-else title="No Posts Found" image="/not-found.jpg" />
    </div>

    <!-- Pagination -->
    <div class="flex justify-center items-center space-x-6">
      <button :disabled="pageNumber <= 1" class="disabled:opacity-50" @click="navigation.previous">
        <Icon
